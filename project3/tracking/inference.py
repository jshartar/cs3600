...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        # get new position distribution for each legal position
        for lastPosition in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, lastPosition))
        #update all possible with beliefs from new position distribution and beliefs
            for item in newPosDist.items():
                if newPosDist[item[0]] > 0:
                    allPossible[item[0]] += newPosDist[item[0]] * self.beliefs[lastPosition]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        count = 0
        # while count is less than the number of particles
        while count < self.numParticles:
            # get each legal position
            for p in self.legalPositions:
                #add the particle to particles[] until count is >= num particles
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        # if the ghost is in jail condition
        if noisyDistance is None:
            for i in range(self.numParticles):
                self.particles[i] = self.getJailPosition()
        # other wise update the beliefs with emission model using the manhattan distance
        else:
            allPossible = util.Counter()
            old = self.getBeliefDistribution()

            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                allPossible[p] += emissionModel[trueDistance] * old[p]
            # if there arent any beliefs condition: then initialize uniformly
            if not any(allPossible.values()):
                self.initializeUniformly(gameState)
            # else update the particles with the sample belief distribution of the beliefs
            else:
                self.particles = [util.sample(allPossible) for p in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        #update the new position distribution using the old position as reference: add as sample distribution
        newParticles = []
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        #make beliefs into a counter object
        beliefs = util.Counter()
        #+= one for each particle then normalize when done
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        #note to self use random!!!!
        self.particles = []
        positions = list(itertools.permutations(self.legalPositions, self.numGhosts))
        random.shuffle(positions)
        i = 0
        # while i counter is less then the number of particles
        while i < self.numParticles:
            # get each position
            for p in positions:
                # add the particle to particles[]
                if i < self.numParticles:
                    self.particles.append(p)
                    i += 1
                #when i >= number of particles break /completed
                else:
                    break
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        #for each particle default probability is 1.0(100%)
        for p in self.particles:
            prob = 1.0
            #for each ghost do conditional: check for None(the ghost is in the jail)
            for ghost in range(self.numGhosts):
                if noisyDistances[ghost] is None:
                    p = self.getParticleWithGhostInJail(p, ghost)
                #if not then update the probability with emission model using manhattan distance
                else:
                    trueDistance = util.manhattanDistance(p[ghost], pacmanPosition)
                    prob *= emissionModels[ghost][trueDistance]
            allPossible[p] += prob
        allPossible.normalize()
        # if the beliefs are empty then initialize the particles again
        if not any(allPossible.values()):
            self.initializeParticles()
            for p in range(self.numParticles):
                particle = self.particles[p]
                # for each particle and ghost check if ghost is in jail (none value)
                for g in range(self.numGhosts):
                    if noisyDistances[g] is None:
                        self.particles[p] = self.getParticleWithGhostInJail(particle, g)
        # for each particle get the sample distribution of the beliefs
        else:
            self.particles = [util.sample(allPossible) for p in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            #for each ghost: get the new position distribution for the ghost
            #  then sample distribution of that for the ghost particle
            for g in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), g,
                                                             self.ghostAgents[g])
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        #repeat like last time: for each particles make it a counter object
        #then normalize and return the beliefs obtained
        beliefs = util.Counter()
        for p in self.particles:
            beliefs[p] += 1.0
        beliefs.normalize()
